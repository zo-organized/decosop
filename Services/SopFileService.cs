...
public class SopFileService
{
    ...
    /// (e.g. a OneDrive-synced folder or network share) instead of the local sop-uploads dir.</summary>
    ...
    public async Task<SopCategory> CreateCategoryAsync(string name, int? parentId = null)
    {
        ...
        var category = new SopCategory { Name = name, SortOrder = maxSort + 1, ParentId = parentId };
    ...
    public async Task RenameCategoryAsync(int id, string newName)
    {
        ...
        {
            var affected = await _db.SopFiles.Where(f => f.StoredFileName.StartsWith(oldPrefix)).ToListAsync();
